...
> 需要说明的是，课程 lab1 所实现的 MapReduce 框架与论文中的细节存在一定出入，因此本文的流程并非与课程实验一一对应。我在本文和 [[领域/计算机/MIT6.5840 Lab 1：MapReduce\|MIT6.5840 Lab 1：MapReduce]] 中分别阐述了两次流程：
> - 本文忠于 MapReduce 论文的原始设计，侧重于流程设计而非实现细节
> - 另一篇文章则在保持流程整体一致的前提下，为满足课程实验的要求做了相应适配。侧重于实现的细节与决策而非流程设计。

**前提**：MapReduce **假设**输入数据是静态的、全量的。必须 Map workers 将所有输入数据全都处理结束后，才能进行下一步（reduce）。

换句话说，它采用**批处理**（batch）方式，而不是流（streaming）
...
**Map Task 1（Worker A）的处理流程**（要注意执行主体。MR 框架是底层框架的行为，Map 函数是用户的行为）

1. MR 框架将 Block 数据块解析为键值对：key="0"（表示偏移量）,value="hello world"
2. MR 框架执行用户定义的 Map 函数，由 Map 遍历每个单词，并 emit 中间键值对：
	- `EmitIntermediate("hello", "1")`
	- `EmitIntermediate("world", "1")`
	- 这些中间键值对先以未分区、未排序的形式写入内存缓冲区（In-Memory Buffer）。此时，框架仅负责顺序存储，不计算分区，也不触发排序。
3. 当内存缓冲区达到阈值后，MR 框架的后台线程会启动溢写操作，将缓冲区内所有中间键值对进行分区，接着在每个分区内部对所有中间键值对进行排序，最后写入本地磁盘。同一分区的多个溢写文件会在 Map 任务结束前会进行归并排序，写入中间文件
	- `hash("hello") % 2 = 0` → 进入分区 0
	- `hash("world") % 2 = 1` → 进入分区 1

> MapReduce 的代码实现是闭源的，上述流程参照了 Hadoop，即主流的、最符合论文思想的开源实现。
> 
> 一个 Map 任务产生一个中间文件和一个索引文件；一个中间文件包含多个分区，分区位置由索引文件维护；每个分区内部是局部有序的
...
- Map Worker 将本地中间文件的路径，以及索引文件中各分区的起始偏移量和长度报告给 Master。
- Master 存储这些信息。当调度 Reduce 任务时，Master 将属于该分区的所有 `<文件路径, 偏移量, 长度>` 元组通知给 Reduce Worker。
- Reduce Worker 收到通知后，主动发起 RPC 请求，去 Map Worker 的本地磁盘拉取（Pull）指定偏移量和长度的数据块。
...
3. 备份任务（Backup Tasks）
	- 当计算接近完成时，Master 会为剩余未完成的 in-progress 任务启动备份执行（即 speculative execution）。
	- 无论是主执行还是备份执行，谁先完成，该任务就标记为完成。这有效解决了因磁盘坏道、CPU 争用等导致的个别机器“拖后腿”问题，直接均衡了最终完成时间。
...
1. Worker 故障（Worker Failure）
	...
	- 处理方式：
		...
		- 已完成（completed）的 Reduce 任务：不需要重新执行。因为 Reduce 的输出存储在全局文件系统（GFS）中，其她机器可访问。
